...
from run_colav.ship_in_transit.utils.utils import EulerInt, ShipDraw
...
class SimpleShipModel(BaseShipModel):
    ''' 
        Creates a ship model object that can be used to simulate a ship in transit, used 
        particularly for collision avoidance purposes.

        The ships model is controlled by a PID speed controller and steered by a rudder. 
    ...
    def __init__(self, ship_config: ShipConfiguration, 
        ...
        self.rudder_config = RudderConfiguration(
            rudder_angle_to_sway_force_coefficient=rudder_config.rudder_angle_to_sway_force_coefficient,
            rudder_angle_to_yaw_force_coefficient=rudder_config.rudder_angle_to_yaw_force_coefficient,
            max_rudder_angle_degrees=rudder_config.max_rudder_angle_degrees
        )
    ...
    def rudder(self, delta):
        ...
        v_force = -self.rudder_config.rudder_angle_to_sway_force_coefficient * delta * (self.forward_speed - u_c)
        r_force = -self.rudder_config.rudder_angle_to_yaw_force_coefficient * delta * (self.forward_speed - u_c)
        return v_force, r_force

    def update_differentials(self, thrust_force=None, rudder_angle=None, *args, **kwargs):
        ''' This method should be called in the simulation loop. It will
            update the full differential equation of the ship.
        '''
        self.three_dof_kinematics()
        self.three_dof_kinetics(thrust_force=thrust_force, rudder_angle=rudder_angle)
    ...
    def store_simulation_data(self, thrust_force, rudder_angle, e_ct, e_psi):
        ...
        self.simulation_results['thrust force [kN]'].append(thrust_force)
