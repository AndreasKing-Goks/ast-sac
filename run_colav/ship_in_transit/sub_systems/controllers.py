...
from run_colav.ship_in_transit.sub_systems.LOS_guidance import NavigationSystem 
...
class SpeedControllerGains(NamedTuple):
    kp: float
    kd: float
    ki: float
...
class ThrustFromSpeedSetPoint:
    """
    Calculates ship thrust based on the desired speed
    """
    def __init__(self,
                 gains: SpeedControllerGains,
                 max_thrust: float,
                 time_step: float
                 ):
        # Initial internal attribute
        self.ship_speed_controller = PidController(
            kp=gains.kp, ki=gains.ki, kd=gains.kd, time_step=time_step
        )
    ...
    def record_initial_parameters(self):
        ...
        '''
        self._initial_state = {
            'max_thrust': copy.deepcopy(self.max_thrust)
        }
        
    def thrust(self, speed_set_point, measured_speed):
        desired_speed = self.ship_speed_controller.pid_ctrl(setpoint=speed_set_point, measurement=measured_speed)
        return self.ship_speed_controller.sat(val=desired_speed, low=-self.max_thrust, hi=self.max_thrust)
    ...
    def reset(self):
        ''' 
            ...
            its initial values
        '''
        self.max_thrust = copy.deepcopy(self._initial_state['max_thrust'])
