...
def clip_magnitude(value, magnitude):
    '''
    Clip the magnitude of value to be within some arbitraty value.
...
def dot_map_dict_to_nested_dict(dot_map_dict):
    ...
    ```
    into its corresponding nested dictionary.
...
def map_recursive(fctn, x_or_iterable):
    ...
    param:
    - fctn: Function from element of iterable to something.
    - x_or_iterable: An element or an Iterable of an element.
    
    return: 
    The same (potentially recursive) iterable but with
...
def filter_recursive(x_or_iterable):
    ...
    param:
    - x_or_iterable: An element or a list.
    
    return:
    - If x_or_iterable is not an Iterable, then return x_or_iterable.
