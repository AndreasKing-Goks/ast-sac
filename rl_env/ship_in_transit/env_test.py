...
from rl_env.ship_in_transit.reward_functions import reward_function
...
class MultiShipRLEnv(Env):
    ...
    def __init__(self, 
        ...
        self.initial_state = np.array([self.test.ship_model.north, self.test.ship_model.east, 0.0,
                                       self.obs.ship_model.north, self.obs.ship_model.east, self.obs.ship_model.yaw_angle, 0.0, self.obs.ship_model.init_forward_speed], dtype=np.float32)
        self.state = self.initial_state
        
        self.obs_dim = len(self.initial_state)
        self.action_dim = 1
