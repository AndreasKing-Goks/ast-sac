...
from ast_sac.core.logging import logger
from ast_sac.launchers import conf
from ast_sac.torch.utils.pytorch_util import set_gpu_mode
import ast_sac.utils.pythonplusplus as ppp
...
def run_experiment_here(
    ...
    param:
    - experiment_function:  Function. `variant` will be passed in as itsonly argument.
    - exp_prefix: Experiment prefix for the save file.
    - variant: Dictionary passed in to `experiment_function`.
    - exp_id: Experiment ID. Should be unique across all experiments. Note that one experiment may correspond to multiple seeds,.
    - seed: Seed used for this experiment.
    - use_gpu: Run with GPU. By default False.
    - script_name: Name of the running script
    - log_dir: If set, set the log directory to this. Otherwise, the directory will be auto-generated based on the exp_prefix.
...
def create_log_dir(
    ...
    param:
    - exp_prefix: All experiments with this prefix will have log directories be under this directory.
    - exp_id: The number of the specific experiment run within this experiment.
    - base_log_dir: The directory where all log should be saved.
...
def setup_logger(
    ...
    param:
    - exp_prefix: The sub-directory for this specific experiment.
    - param variant:
    - param text_log_file:
